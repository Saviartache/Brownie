...
func main() {
	if runInspectionCommand(os.Args[1:]) {
		return
	}
	...
	flag.Usage = func() {
		_, _ = fmt.Fprintf(os.Stderr, "Usage:\n  extractor [flags]\n  extractor diff [flags] OLD_BUILD NEW_BUILD\n  extractor packets [flags] BUILD_OR_METADATA\n  extractor handlers [flags] BUILD [NEW_BUILD]\n\nMain extraction flags:\n")
		flag.PrintDefaults()
	}
	flag.Parse()
...
func runInspectionCommand(args []string) bool {
	if len(args) == 0 {
		return false
	}
	var code int
	switch args[0] {
	case "diff":
		code = toolcli.RunDiff("extractor diff", args[1:], os.Stdout, os.Stderr)
	case "packets":
		code = toolcli.RunPackets("extractor packets", args[1:], os.Stdout, os.Stderr)
	case "handlers":
		code = toolcli.RunHandlers("extractor handlers", args[1:], os.Stdout, os.Stderr)
	default:
		return false
	}
	if code != 0 {
