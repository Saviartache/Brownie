...
func TestCompareFiles(t *testing.T) {
	old := t.TempDir()
	new := t.TempDir()
	writeTree(t, old, map[string]string{"same.bin": "same", "changed.bin": "old", "removed.bin": "bye"})
	writeTree(t, new, map[string]string{"same.bin": "same", "changed.bin": "new", "added.bin": "hi"})

	got, err := CompareFiles(old, new)
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 3 {
	...
	want := map[string]string{"added.bin": "added", "changed.bin": "changed", "removed.bin": "removed"}
	for _, change := range got {
		if want[change.Path] != change.Kind || change.OldHash == change.NewHash && change.Kind == "changed" {
