...
func (p *Pipeline) prepareMetadata(build localsrc.Build, workDir string) string {
	...
	info, err := metadata.Prepare(build.Metadata, build.GameAssembly, dst, metadata.DefaultVersion)
	if err != nil {
		p.Log.Warn("metadata decryption failed: %v", err)
		return ""
	}
	if info.Decrypted {
		p.Log.Success("Recovered XXTEA key %s and decrypted global-metadata.dat -> %s", info.Key, filepath.Base(dst))
	} else {
...
func (p *Pipeline) publishBuild(workDir, publishDir, version, hash string) (builddiff.Diff, gamediff.Summary, error) {
	...
	p.writePacketInventory(workDir)
	handlers := p.writeHandlerInventory(workDir)
	p.writeDetailedBuildReport(publishDir, workDir)
	p.writeHandlerDiff(publishDir, workDir, handlers)
...
// writePacketInventory stores a ready-to-use protocol catalog in every client
// archive and in publish/current (which intentionally omits the native files).
func (p *Pipeline) writePacketInventory(workDir string) {
	if _, err := os.Stat(filepath.Join(workDir, gameFilesDirName, "global-metadata.dat")); err != nil {
		return
	}
	inventory, err := packetreport.Extract(workDir)
	if err != nil {
		p.Log.Warn("packet-name extraction failed: %v", err)
		return
	}
	var textInventory, jsonInventory bytes.Buffer
	if err := inventory.WriteText(&textInventory, "all"); err != nil {
		p.Log.Warn("formatting packet inventory: %v", err)
		return
	}
	if err := inventory.WriteJSON(&jsonInventory); err != nil {
		p.Log.Warn("formatting packet inventory JSON: %v", err)
		return
	}
	if err := os.WriteFile(filepath.Join(workDir, "packets.txt"), textInventory.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing packet inventory: %v", err)
		return
	}
	if err := os.WriteFile(filepath.Join(workDir, "packets.json"), jsonInventory.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing packet inventory JSON: %v", err)
		return
	}
	p.Log.Success("Extracted packet catalog: %d incoming, %d outgoing, %d data", len(inventory.Incoming), len(inventory.Outgoing), len(inventory.Data))
}

// writeHandlerInventory stores managed/native packet gateways, registration
// sites, callbacks, packet IDs, RVAs, and normalized code fingerprints.
func (p *Pipeline) writeHandlerInventory(workDir string) *handlerreport.Report {
	if _, err := os.Stat(filepath.Join(workDir, gameFilesDirName, "GameAssembly.dll")); err != nil {
		return nil
	}
	report, err := handlerreport.Extract(workDir)
	if err != nil {
		p.Log.Warn("packet-handler extraction failed: %v", err)
		return nil
	}
	var textReport, jsonReport bytes.Buffer
	if err := report.WriteText(&textReport); err != nil {
		p.Log.Warn("formatting packet-handler report: %v", err)
		return nil
	}
	if err := report.WriteJSON(&jsonReport); err != nil {
		p.Log.Warn("formatting packet-handler JSON: %v", err)
		return nil
	}
	if err := os.WriteFile(filepath.Join(workDir, "handlers.txt"), textReport.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing packet-handler report: %v", err)
		return nil
	}
	if err := os.WriteFile(filepath.Join(workDir, "handlers.json"), jsonReport.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing packet-handler JSON: %v", err)
		return nil
	}
	p.Log.Success("Recovered %d packet gateways, registration sites, and callbacks", len(report.Functions))
	return &report
}

func (p *Pipeline) writeHandlerDiff(publishDir, workDir string, current *handlerreport.Report) {
	previous := latestArchivedBuild(publishDir)
	if previous == "" || current == nil {
		return
	}
	oldReport, err := handlerreport.Extract(previous)
	if err != nil {
		p.Log.Warn("packet-handler diff failed: %v", err)
		return
	}
	diff := handlerreport.CompareReports(oldReport, *current)
	var textReport, jsonReport bytes.Buffer
	if err := diff.WriteText(&textReport); err != nil {
		p.Log.Warn("formatting packet-handler diff: %v", err)
		return
	}
	if err := diff.WriteJSON(&jsonReport); err != nil {
		p.Log.Warn("formatting packet-handler diff JSON: %v", err)
		return
	}
	if err := os.WriteFile(filepath.Join(workDir, "handler-diff.txt"), textReport.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing packet-handler diff: %v", err)
		return
	}
	if err := os.WriteFile(filepath.Join(workDir, "handler-diff.json"), jsonReport.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing packet-handler diff JSON: %v", err)
		return
	}
	p.Log.Success("Packet-handler diff written (%d changed, %d added, %d removed)", len(diff.Changed), len(diff.Added), len(diff.Removed))
}

// writeDetailedBuildReport compares the work tree against the newest immutable
// archive and stores both human- and machine-readable semantic reports. Report
// failures are non-fatal so an old/incomplete archive cannot block publishing.
func (p *Pipeline) writeDetailedBuildReport(publishDir, workDir string) {
	previous := latestArchivedBuild(publishDir)
	if previous == "" {
		return
	}
	p.Log.Info("Building detailed diff against %s...", filepath.Base(previous))
	report, err := buildreport.Compare(previous, workDir, buildreport.Options{})
	if err != nil {
		p.Log.Warn("detailed build diff failed: %v", err)
		return
	}
	var textReport, jsonReport bytes.Buffer
	if err := report.WriteText(&textReport, buildreport.TextOptions{FileLimit: 100}); err != nil {
		p.Log.Warn("formatting detailed build diff: %v", err)
		return
	}
	if err := report.WriteJSON(&jsonReport); err != nil {
		p.Log.Warn("formatting detailed build diff JSON: %v", err)
		return
	}
	if err := os.WriteFile(filepath.Join(workDir, "build-diff.txt"), textReport.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing detailed build diff: %v", err)
		return
	}
	if err := os.WriteFile(filepath.Join(workDir, "build-diff.json"), jsonReport.Bytes(), 0o644); err != nil {
		p.Log.Warn("writing detailed build diff JSON: %v", err)
		return
	}
	p.Log.Success("Detailed build diff written (packets, types, literals, and files)")
}

func latestArchivedBuild(publishDir string) string {
	...
	for _, entry := range entries {
		...
		path := filepath.Join(publishDir, entry.Name())
		if _, err := os.Stat(filepath.Join(path, gameFilesDirName, "global-metadata.dat")); err != nil {
			continue
		}
		info, err := entry.Info()
		if err == nil {
