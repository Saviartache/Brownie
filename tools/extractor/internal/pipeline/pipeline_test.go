...
func TestLatestArchivedBuild(t *testing.T) {
	publishDir := t.TempDir()
	makeBuild := func(name string, age time.Duration, metadata bool) string {
		path := filepath.Join(publishDir, name)
		if metadata {
			gameFiles := filepath.Join(path, gameFilesDirName)
			if err := os.MkdirAll(gameFiles, 0o755); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(filepath.Join(gameFiles, "global-metadata.dat"), []byte("x"), 0o644); err != nil {
				t.Fatal(err)
			}
	...
	makeBuild("current", 0, true)
	makeBuild("incomplete", 30*time.Minute, false)
	want := makeBuild("newest-archive", time.Hour, true)
	makeBuild("old-archive", 2*time.Hour, true)
	if got := latestArchivedBuild(publishDir); got != want {
		t.Fatalf("latestArchivedBuild = %q, want %q", got, want)
	}
}
