...
| A turret that fires and can never be hurt keeps a point blank, refused like damaging ground | It is no body to keep room from — `<Invincible/>` or sent with no health, and usually drawn as nothing — so nothing kept the planner off it, and a shot fired beside the character lands before any step aside. How far is worked out from its own shots: the client's motion code, flown for a command lead and the step out of their square | dropping them as spawners, which is what the reference does — its enemy snapshot discards every hidden helper and every object with no health, so its dodge never sees one; one fixed radius round everything that shoots |
