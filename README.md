...
is about 90 MB and only ever as new as the Node that built it. That binary also
decides the download's size, and the archive is **a zip compressed with LZMA**
because of it: deflate leaves the release at 38 MB and LZMA at 25. Say so
wherever it is posted — LZMA is in the zip specification but not in Windows'
own reader, so Explorer refuses the file and 7-Zip or WinRAR opens it.
